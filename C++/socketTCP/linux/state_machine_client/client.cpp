...
void socketTCPClient::sendFile(const I1 *path)
{
  ...
  sendData(file_buf.get(),u4_fileLen,SENDFILE);
}

void socketTCPClient::sendData(const I1* data,const U4 &u4_dataSize,const U2 u2_dataType)
{
  ...
  char buffer[12] = { "verify" };
  memcpy(buffer + 6, &u4_dataSize, sizeof(u4_dataSize));
  memcpy(buffer + 10, &u2_dataType, sizeof(u2_dataType));
...
void socketTCPClient::sendFixedStruct(const FIXED_LENGTH_STRUCT &fixedStruct)
{
  ...
  sendData(buffer.get(),sizeof(FIXED_LENGTH_STRUCT),FIXED_STRUCT);
}
...
void socketTCPClient::sendMutableStruct(const MUTABLE_LENGTH_STRUCT &mutableStruct)
{
  ...
  sendData(buffer.get(),u4_bufferSize,MUTABLE_STRUCT);
}
